...
bot.on("message", async message => {
  ...
  if (cmd == `${prefix}setmessagechannel`) {
    if (!message.member.roles.find('name', 'Bot Commander')) return message.channel.send(":x: You don't have the enough permissions!");
    var chid = args[0];
    con.query(`SELECT * FROM botconfig WHERE name = 'messagechannel'`, (err, rows) => {
      let sql = `UPDATE botconfig SET value = '${chid}' WHERE name = 'messagechannel'`;
  ...
  if (cmd == `${prefix}setjoinmessage`) {
    if (!message.member.roles.find('name', 'Bot Commander')) return message.channel.send(":x: You don't have the enough permissions!");
    var jmsg = args.join(" ");
    con.query(`SELECT * FROM botconfig WHERE name = 'joinmessage'`, (err, rows) => {
      let sql = `UPDATE botconfig SET value = '${jmsg}' WHERE name = 'joinmessage'`;
      con.query(sql, console.log);
  ...
  if (cmd == `${prefix}setleavemessage`) {
    if (!message.member.roles.find('name', 'Bot Commander')) return message.channel.send(":x: You don't have the enough permissions!");
    var lmsg = args.join(" ");
    con.query(`SELECT * FROM botconfig WHERE name = 'leavemessage'`, (err, rows) => {
      let sql = `UPDATE botconfig SET value = '${lmsg}' WHERE name = 'leavemessage'`;
      con.query(sql, console.log);
  ...
  if (cmd == `${prefix}setdmmessage`) {
    if (!message.member.roles.find('name', 'Bot Commander')) return message.channel.send(":x: You don't have the enough permissions!");
    var dmmsg = args.join(" ");
    con.query(`SELECT * FROM botconfig WHERE name = 'dmmessage'`, (err, rows) => {
      let sql = `UPDATE botconfig SET value = '${dmmsg}' WHERE name = 'dmmessage'`;
      con.query(sql, console.log);
